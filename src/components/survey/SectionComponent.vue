...
<div class="p-p-0">
    ...
    <div class="p-px-2" v-if="item.direct_indicator" style="background-color: white; border-radius: 5px;" :style="[(active) ? 'border: 1px solid blue;' : 'border: 1px solid lightgrey', ((requireStatus || item.required) && (!active)) ? 'border: 1px solid red' : '', hover ? 'background-color: #F1F1F1;' : '']" @mouseover="hover = true" @mouseleave="hover = false">
        ...
        <!-- {{requireStatus}} {{active}} {{answer}} {{item}} -->
        <answer-input
        :value="answer"
...
export default {
    ...
    methods: {
        ...
        checkRequirementStatus () {
            console.log('check Requirements', this.answer)
