...
<div class="p-p-0">
    ...
    <div class="p-px-2" v-if="item.direct_indicator" style="background-color: white; border-radius: 5px;" :style="[(active) ? 'border: 1px solid blue;' : 'border: 1px solid lightgrey', ((requireStatus || item.required) && (!active)) ? 'border: 1px solid red' : '']">
    ...
    methods: {
        ...
        checkRequirementStatus () {
            if (this.focused && this.item.isMandatory) {
                if (!this.answer?.[0].length && this.answer?.[1] === '' && !this.item.text) {
