<template>
    ...
    <div v-else-if="methods.length">
         <div v-if="table" class="p-grid p-m-5" style="height: 500px;">
...
<script>
    ...
    export default {
            ...
            filteredMethods () {
                return this.methods.filter(method => {
                    return (
                        ...
                        method.description.toLowerCase().includes(this.search.toLowerCase())
                    )
                })
            }
        },
        ...
        methods: {
            ...mapActions('method', ['setMethod', 'deleteMethod']),
            goTomethod (method) {
                if (method?.data) {
                    method = method.data
                }
                this.$emit('clicked-method', method)
            },
            destroyMethod () {
